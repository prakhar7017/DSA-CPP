...
public:
    ...
    TreeNode* solve(int preStart,int postStart,int preEnd,vector<int>& preorder, vector<int>& postorder,unordered_map<int,int>&mp){
        ...
        int noofnodeToLeft=j-postStart+1;

        root->left=solve(preStart+1,postStart,preStart+noofnodeToLeft,preorder,postorder,mp);
        root->right=solve(preStart+noofnodeToLeft+1,j+1,preEnd,preorder,postorder,mp);
        return root;
