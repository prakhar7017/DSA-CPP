...
public:
    int solve(vector<int>& prices,int i,int buy,int fee,vector<vector<int>>&dp){
        ...
        int profit=INT_MIN;
        if(buy){
            int letsBuy=-prices[i]+solve(prices,i+1,0,fee,dp);
            int letsIgnore=0+solve(prices,i+1,1,fee,dp);
            profit=max(letsBuy,letsIgnore);
        }else{
            int letsSell=prices[i]-fee +solve(prices,i+1,1,fee,dp);
            int letsIgnore=solve(prices,i+1,0,fee,dp);
        ...
        for(int i=n-1;i>=0;i--){
            for(int buy=0;buy<=1;buy++){
                ...
                if(buy){
                    ...
                    int letsSell=prices[i]-fee+dp[i+1][1];//solve(prices,i+1,1,dp);
    ...
    int spaceOptmized(vector<int>& prices,int fee){
        int n=prices.size();
        ...
        for(int i=n-1;i>=0;i--){
            for(int buy=0;buy<=1;buy++){
                ...
                if(buy){
                    ...
                    int letsSell=prices[i]-fee+nextRow[1];//solve(prices,i+1,1,dp);
    ...
    int maxProfit(vector<int>& prices, int fee) {
        ...
        // return spaceOptmized(prices,fee);
    }
};
